...
const matrixBfs = (graph, visited, startNode) => {
    ...
    while (queue.length > 0) {
        ...
        for (
            let neighborNode = 1;
            neighborNode < graph[currentNode].length;
            neighborNode++
        ) {
...
const dfs = (graph, visited, currentNode) => {
    visited[currentNode] = true;
    for (let i = 0; i < graph[currentNode].length; i++) {
        ...
        dfs(graph, visited, neighborNode);
    }
}
const visited3 = [null, false, false, false, false]
dfs(listGraph, visited3, 1)

///////////////////////////////////////

const bfs = (graph, visited, startNode) => {
    const queue = new Queue();
    queue.enqueue(startNode);
    visited[startNode] = true;
    console.log(`${startNode}방문했음~~`);

    while (queue.length !== 0) {
        const exitNode = queue.dequeue();
        for (let i = 0; i < graph[exitNode].length; i++) {
            const neighborNode = graph[exitNode][i]
            if (visited[neighborNode] === true) {
                continue
            }
            queue.enqueue(neighborNode)
            visited[neighborNode] = true;
            console.log(`${neighborNode}방문했음~~`);
        }
    }
}

const visited4 = [null, false, false, false, false]
bfs(listGraph, visited4, 1)
